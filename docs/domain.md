...
aws sagemaker list-apps --domain-id-equals "d-vdtztzq1kgz8" \
  --query 'Apps[].[AppType,AppName,SpaceName,UserProfileName,Status]' --output table

# ---------------------------------------------------------------------
# |                             ListApps                              |
# +------------+----------+--------------------+--------+-------------+
# |  Canvas    |  default |  None              |  alice |  Failed     |
# |  JupyterLab|  default |  alice-jupyterlab  |  None  |  InService  |
# +------------+----------+--------------------+--------+-------------+

aws sagemaker delete-app --domain-id "d-vdtztzq1kgz8" --space-name alice-jupyterlab \
...
# sagemaker-domain-dev-bike-sharing-rf

for arn in $(aws sagemaker list-model-packages --model-package-group-name "sagemaker-domain-dev-bike-sharing-rf" \
